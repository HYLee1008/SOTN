...
parser.add_argument('--matedScoresFile', help='filename for the mated scores', type=str, default='mated.txt')
parser.add_argument('--nonMatedScoresFile', help='filename for the non-mated scores', type=str, default='nonmated.txt')
parser.add_argument('--genuineScoresFile', help='filename for the genuine scores', type=str, default='genuine.txt')
parser.add_argument('--figureFile', help='filename for the output figure', type=str, default='revok.jpg')
...
genuineScoresFile = args.genuineScoresFile
...
genuineScores = numpy.loadtxt(genuineScoresFile)

if nBins == -1:
	nBins = min(len(matedScores)/10,100)

# define range of scores to compute D
bin_edges = numpy.linspace(min([min(matedScores), min(nonMatedScores), min(genuineScores)]), max([max(matedScores), max(nonMatedScores), max(genuineScores)]), num=nBins + 1, endpoint=True)
...
y3 = numpy.histogram(genuineScores, bins=bin_edges, density = True)[0]
...
ax = sns.kdeplot(nonMatedScores, shade=False, label='Imposter', color=sns.xkcd_rgb["pale red"])
x2,y2 = ax.get_lines()[1].get_data()
ax = sns.kdeplot(genuineScores, shade=False, label='Genuine', color=sns.xkcd_rgb["medium blue"])
