...
parser.add_argument('--figureFile', help='filename for the output figure', type=str, default='result.jpg')
...
ax = sns.kdeplot(nonMatedScores, shade=False, label='Non-Mated', color=sns.xkcd_rgb["pale red"])
x2,y2 = ax.get_lines()[1].get_data()

ax2 = ax.twinx()
lns3, = ax2.plot(bin_centers, D, label='$\mathrm{D}_{\leftrightarrow}(s)$', color=sns.xkcd_rgb["medium blue"])

# print omega * LR = 1 lines
index = numpy.where(D <= 0)
ax.axvline(bin_centers[index[0][0]], color='k')
